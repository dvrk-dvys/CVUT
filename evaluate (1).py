...
def evaluate(rewards: List[float]):
    ...
    plot_series(simple_moving_average(rewards, 1000))
    # plot_series(exponential_moving_average(rewards, 0.2))
    print("Last 10% average")
    print(np.mean(rewards[-int(len(rewards)*0.1):]))
